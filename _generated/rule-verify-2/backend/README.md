# rule-verify-2 FastAPI Backend

A enterprise fullstack backend built with FastAPI.
...
```bash
# Build image
docker build -t rule-verify-2-backend .

# Run container
docker run -d \
  --name rule-verify-2-api \
  -p 8000:8000 \
  -e DATABASE_URL=postgresql://user:pass@db/dbname \
  rule-verify-2-backend
...
Copyright © {{YEAR}} rule-verify-2. All rights reserved.