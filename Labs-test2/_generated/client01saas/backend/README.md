# client01saas FastAPI Backend

A real_estate_saas fullstack backend built with FastAPI.
...
```bash
# Build image
docker build -t client01saas-backend .

# Run container
docker run -d \
  --name client01saas-api \
  -p 8000:8000 \
  -e DATABASE_URL=postgresql://user:pass@db/dbname \
  client01saas-backend
...
Copyright © {{YEAR}} client01saas. All rights reserved.