...
- Multi-tenant PostgreSQL schema with `org_id` on scoped tables and row-level security policies enforced per role.
- FastAPI backend exposing modular routers for auth, tenants, units, payments, tickets, and AI summaries.
- Next.js frontend leveraging server components for dashboards and client components for interactive CRUD flows.
- Celery workers orchestrating monthly AI summaries, PDF generation, and notification stubs.
- Docker Compose stack bundling web, API, worker, and Postgres services behind shared environment configuration.
